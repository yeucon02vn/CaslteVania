...
void Sprite::Render(Camera *cam)
{
	if (spriteHandle && texture)
	{
		...
		spriteHandle->Draw(texture, NULL, NULL, &cam->SetPositionInCamera(position), color);
...
void Sprite::Render(RECT rect, bool isLeft, int width, int height, Camera *cam)
{
	if (spriteHandle && texture)
	{
		D3DXVECTOR3 pos = cam->SetPositionInCamera(position);
...
void Sprite::Render(D3DXVECTOR3 position, D3DCOLOR color, Camera *cam)
{
	if (spriteHandle && texture)
	{
		spriteHandle->Begin(D3DXSPRITE_ALPHABLEND);
